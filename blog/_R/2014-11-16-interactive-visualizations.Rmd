...
TODO
  - one rChart and one ggvis not showing…
	- list technical problems
http://cran.r-project.org/web/packages/knitr/vignettes/knit_print.html
check knit hooks

	- try plotly in jekyll!
	- add CC figure
	- need three header levels in homepages
	- switch to permalinks
...
* **plotly:** Embedding plotly charts into R markdown documents did not work as shown [here][plotly-rmd], but adding `session="knit"` to the `ggplotly()` call [solved the issue][plotly-issue] (thanks to Scott Chamberlain and Marianne Corvellec for help!). FIXME: check that this works with jekyll too.
...
The whole process of producing valid interactive html charts from R markdown is a respectable things, and I'm kind of amazed that I managed to implement it even this well.

I also noticed several minor issues:

* googleVis was missing axis labels by default
* rCharts is missing legend titles, and behaves strangely on scatter plot: legend shows incorrect information, and the plot area is too tight 
